                ...
                for queue_message in queue_messages:
                    ...
                    for item in links:
                        ...
                        filename = (
                            link.split("=")[-1]
                            + "_"
                            + str(starttime.time().strftime("%Y-%m-%d-%H-%M-%S"))
                        )
                        # we will store all the results as a list of dictionaries
                        while cap.isOpened():
                            ...
                            if success:
                                ...
                                for box in json.loads(results[0].tojson()):
                                    ...
                                    box["full_process_speed"] = sum(
                                        results[0].speed.values()
                                    )
                                    ...
                                    if (
                                        ...
                                        save_df(df, filename, result_blob_client)
                                        summary(df, filename, result_blob_client)
                            ...
                            # Break the loop if the process should not continue
                                ...
                                save_df(df, filename, result_blob_client)
                                summary(
                                    df, filename, result_blob_client, processing_time
                                )
                                break
